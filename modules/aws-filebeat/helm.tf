resource "helm_release" "aws-filebeat" {
  ...
  # Add Annotations (to assign IAM roles and such)
  dynamic "set" {
...
setup:
  ...
  ilm.rollover_alias: "filebeat-${var.aws_log_source}"
  template.name: "filebeat-${var.aws_log_source}"
  template.pattern: "filebeat-${var.aws_log_source}-*"
